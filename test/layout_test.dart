...
Future<void> selectCardCount(WidgetTester tester, int count) async {
  ...
  await tester.tap(find.text(count == 1 ? '1 card' : '$count cards').last);
  await tester.pumpAndSettle();
}

void main() {
  for (final MapEntry<String, Size> screen in screens.entries) {
    for (final int count in <int>[1, 2, 4, 7, kMaxCards]) {
      testWidgets('$count cards lay out on a ${screen.key} without overflow', (
        ...
        // Fill every slot with a real cartela and render the boards. Slots
        // below the fold have to be scrolled to first on the smaller screens.
        for (int slot = 0; slot < count; slot++) {
          final Finder play = find.text('Play').first;
